...
function SessionStatus({ error }: { error?: boolean }) {
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0d0f10] px-6">
      <div aria-hidden="true" className="faulttrace-grid pointer-events-none absolute inset-0 opacity-35" />
      <section className="relative w-full max-w-lg rounded-lg border border-white/10 bg-[#151719]/95 p-8 text-center shadow-sm">
        <div className="flex justify-center"><BrandMark /></div>
        <p className="mt-7 text-xs font-semibold uppercase tracking-[0.18em] text-teal-300">
          Secure workspace
        </p>
        ...
        {error && (
          <a href="/sign-in" className="mt-6 inline-flex rounded-md bg-teal-300 px-5 py-3 text-sm font-bold text-[#0d0f10] hover:bg-teal-200">
            Return to sign in
          </a>
        )}
      </section>
