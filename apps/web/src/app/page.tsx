    ...
    <main className="relative min-h-screen overflow-hidden bg-[#0d0f10]">
      ...
      <div className="relative mx-auto max-w-7xl px-6 sm:px-10">
        <header className="flex items-center justify-between border-b border-white/10 py-7">
          ...
          <Link
            href="/sign-in"
            className="cursor-pointer rounded-md border border-teal-300/30 bg-teal-300/10 px-5 py-2.5 text-sm font-semibold text-teal-100 transition hover:border-teal-300/70 hover:bg-teal-300/15"
          >
        ...
        <section className="grid gap-14 py-20 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:gap-16 lg:py-28">
          <div>
            <p className="mb-6 inline-flex items-center gap-2 rounded-full border border-teal-300/20 bg-teal-300/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-teal-300">
            ...
            <h1 className="max-w-3xl text-5xl font-semibold leading-[1.08] tracking-tight text-white sm:text-6xl xl:text-7xl">
              Trace the fault.
              <span className="block text-teal-300">Trust the source.</span>
            </h1>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-5">
              <Link
                href="/sign-in"
                className="inline-flex items-center gap-3 rounded-md bg-teal-300 px-6 py-3.5 text-sm font-bold text-[#0d0f10] transition hover:bg-teal-200"
              >
                Open workspace <span aria-hidden="true">→</span>
              </Link>
              <span className="text-sm text-zinc-400">Access is for invited teams.</span>
            </div>
          </div>

          <div className="rounded-lg border border-white/10 bg-[#151719] p-5 shadow-sm sm:p-7">
            <div className="flex items-center justify-between gap-3 border-b border-white/10 pb-5">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-300">Workflow preview</p>
            ...
            <ol className="mt-6 space-y-4">
              {preview.map(([number, title, detail]) => (
                <li key={number} className="flex gap-4 rounded-lg border border-white/10 bg-white/[0.03] p-4">
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-md border border-teal-300/20 bg-teal-300/10 text-xs font-bold text-teal-300">
                    {number}
                  </span>
                  <div>
        ...
        <section aria-labelledby="workflow-title" className="border-t border-white/10 pb-20 pt-12">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-300">The FaultTrace path</p>
              <h2 id="workflow-title" className="mt-3 text-3xl font-semibold tracking-tight text-white">Built for the work on site</h2>
            </div>
            <p className="max-w-md text-sm leading-6 text-zinc-400">A clear path from the first symptom to a documented fix.</p>
          </div>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {steps.map((step) => (
              <article key={step.number} className="rounded-lg border border-white/10 bg-[#151719]/75 p-6">
                <span className="text-sm font-bold text-teal-300">{step.number}</span>
